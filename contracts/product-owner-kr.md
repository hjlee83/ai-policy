...
명확화 질문과 승인 요청은 기본적으로 현재 세션에서 직접 한다. 다르게 확인되지 않는 한 현재 세션이
유일한 인터페이스다. Target Repository나 배포 설정에서 이 역할이 사용할 수 있는 별도의 오케스트레이터
채널(예: Slack)을 명시적으로 지정한 경우에만 그 채널로 질문과 승인을 라우팅한다. 오케스트레이터
채널이 있다고 임의로 가정하지 않으며, 사용자가 볼 수 있다고 확인되지 않은 곳에서 응답을 기다리지
않는다. 오케스트레이터 채널이 설정되어 있지 않으면, 이 계약의 다른 곳에서 언급하는 의사결정 채널은
모두 현재 세션을 가리키는 것으로 취급한다.
