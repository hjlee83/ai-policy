...
4. 다음 내용을 확인한다.
   ...
   - Verification Gates
   - Out of Scope
5. 대상 저장소에 `ARCHITECTURE.md`가 있으면 코드 분석 전에 먼저 읽는다.
...
- Verification 완료
- 구조(모듈 구성, 데이터 흐름, 컴포넌트 책임)에 영향을 준 변경이면 ARCHITECTURE.md를 갱신했거나, 없고 이번이 최초의 의미 있는 구조 변경이면 최소 버전을 새로 작성했다
- Pull Request 작성 완료
