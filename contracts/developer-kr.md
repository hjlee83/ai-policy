...
`spec.md`의 모호성 때문에 작업을 계속할 수 없으면 추측하거나 사용자에게 직접 질문하지 않는다.
...
`report.md`가 준비되고 `status.md`가 갱신되면 결과를 오케스트레이터에게 보고한다: Target
...
규칙 참고). task 폴더 경로, 결과 `status.md` 상태, 구현 내용을 한 줄로 요약해 포함한다. 이 보고를
