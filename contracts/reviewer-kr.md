...
프로젝트의 task 폴더 루트(`~/task/<project-name>/`)에 공유 `summary.md`가 있다면 탐색 출발점으로만 취급한다. 이 문서는 검증의 대체물이 아니므로 실제 diff와 코드로 정확성을 확인한다.
...
- 문서화
- 단순화
- 구조(모듈 구성, 데이터 흐름, 컴포넌트 책임)에 영향을 준 변경인데 프로젝트의 `~/task/<project-name>/summary.md`가 갱신되지 않은 경우
