...
`merge.md`가 기록되고 `status.md`가 갱신되면 결과를 오케스트레이터에게 보고한다: Target
...
규칙 참고). task 폴더 경로, 결과 `status.md` 상태, 병합 상태(MERGED / BLOCKED / RETRY)를
포함한다. 이 보고를 보내기 전까지는 병합이 끝난 것으로 취급하지 않는다.
